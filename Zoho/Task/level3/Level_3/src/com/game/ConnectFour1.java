...
public class ConnectFour1 {
    ...
    private static int findBestMove(char[][] board) {
        for (int col = 0; col < COLS; col++) {
            if (canWinNextMove(board, col, PLAYER_TWO)) {
                return col;
            }
        }
        for (int col = 0; col < COLS; col++) {
            if (canWinNextMove(board, col, PLAYER_ONE)) {
                return col;
            }
        }
        for (int col = 0; col < COLS; col++) {
    ...
    private static boolean canWinNextMove(char[][] board, int col, char disc) {
        if (!isValidMove(board, col)) {
            return false;
        }
        int row = getNextOpenRow(board, col);
        board[row][col] = disc;
        boolean canWin = checkForWin(board, disc);
    ...
    private static int getNextOpenRow(char[][] board, int col) {
        for (int i = ROWS - 1; i >= 0; i--) {
            if (board[i][col] == EMPTY_SLOT) {
                return i;
            }
